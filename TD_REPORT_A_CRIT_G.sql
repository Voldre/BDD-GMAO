-- Rapport du suivi de dispo des moyens goulots à l'année
WITH
...
SELECT RR.NOM, (3168 - RR.HEURES)/3168, RR.HEURES, RR.NB_DI, 
     RR.HEURES / RR.NB_DI , (3168 - RR.HEURES) / RR.NB_DI
     --      MTTR                           MTBF
FROM
 (
 SELECT(COALESCE(SUM(R2.HEURES_A_LONG),0)  +
       ...
       , SUM(R0.NB_DI) AS NB_DI, R0.NOM AS NOM
FROM

(SELECT COUNT(*) AS NB_DI ,   NOM   FROM MYTABLE) R0,
