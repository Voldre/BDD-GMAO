...
-- WHERE MAT.CRITICALITY in ('A')  -- Pour l'instant pas besoin
  ...
  WHERE WO.STATUS_CODE = 'CLOSED' -- Pour toutes les DI avec un OT soldé cette semaine
...
MYTABLE_2(StartDate,EndDate, NOM) AS
(SELECT MR.CREATIONDATE AS StartDate, 
	TO_DATE( (SYSDATE - TO_CHAR(SYSDATE,'D') + 5) ) AS EndDate,
	MAT.ID AS NOM

FROM CSWO_MR MR 
   INNER JOIN CSWO_MREQPT WE
   ON WE.MR_ID = MR.ID  
     INNER JOIN CSEQ_EQUIPMENT EQ
     ON EQ.ID = WE.EQPT_ID
    INNER JOIN CSEQ_MATERIAL MAT
...
-- SANS OT
LEFT JOIN CSWO_WO WO ON MR.WO_ID = WO.ID
WHERE WO.ID IS NULL
-- ON PREND TOUT L'ENSEMBLE MR.ID QUI NE CONTIENT PAS WO.ID
...
-- Chevauchement intervalle si OT
...
-- Chevauchement intervalle si PAS D'OT
MYTABLE2_2(StartDate,EndDate, NOM) AS
(SELECT   s1.StartDate,
      MIN(t1.EndDate) AS EndDate, s1.NOM
FROM MYTABLE_2 s1 
INNER JOIN MYTABLE_2 t1 ON s1.StartDate <= t1.EndDate
     AND NOT EXISTS(SELECT * FROM MYTABLE_2 t2 
        WHERE t1.EndDate >= t2.StartDate AND t1.EndDate < t2.EndDate) 
WHERE NOT EXISTS(SELECT * FROM MYTABLE_2 s2 
    WHERE s1.StartDate > s2.StartDate 
    AND s1.StartDate <= s2.EndDate ) 
GROUP BY s1.StartDate, s1.NOM)
,

MYTABLE3(StartDate,EndDate, NOM) AS
(SELECT StartDate, EndDate, NOM FROM MYTABLE2
 UNION
 SELECT StartDate, EndDate, NOM FROM MYTABLE2_2 )


TABLE_R0(NOM, NB_DI) AS
(SELECT NOM , COUNT(*) AS NB_DI  
FROM MYTABLE, MYTABLE_2 GROUP BY NOM)
,
